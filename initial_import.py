        ...
        while True:
            try:
                resp = slack.conversations_history(
                    channel=channel, limit=50, cursor=cursor, oldest=oldest_ts
                )
                break
                ...
                if (
                    ...
                    logging.info(
                        f"Rate limited fetching history for {channel}, retrying after {retry_after}s"
                    )
...
def main():
    ...
    import_posts(
        db, [(ts, QUESTION_CH, user, text, ts) for ts, user, text in qa_threads]
    )
        ...
        while True:
            try:
                resp = slack.conversations_replies(
                    channel=QUESTION_CH, ts=thread_ts, limit=100
                )
                ...
                if (
                    ...
                    retry_after = int(e.response.headers.get("Retry-After", 2))
                    logging.info(
                        f"Rate limited when fetching replies for thread {thread_ts}, retrying after {retry_after}s"
                    )
                    time.sleep(retry_after)
                    retry_count += 1
                    continue
                    ...
                    logging.warning(
                        f"Failed fetching replies for thread {thread_ts}: {e}"
                    )
                    break
