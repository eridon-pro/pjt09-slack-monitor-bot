...
from utils.db import (
    get_unjudged_reactions,
    cache_positive_reaction,
    get_unscored_positive_reactions,
    apply_reaction_scores,
)
...
def judge_positive_reaction(reaction_name: str) -> bool:
    ...
    if not openai.api_key:
        logger.info(
            f"[LLM: {MODEL}] judge_positive_reaction skipped (no API key): {reaction_name}"
        )
    ...
    try:
        ...
        logger.info(
            f"[LLM: {MODEL}] judge_positive_reaction resp for ':{reaction_name}:': '{content}'"
        )
        return content == "yes"
...
def apply_all_positive_reactions():
    ...
    for reaction in unjudged:
        ...
        logger.info(
            f"[LLM-batch: {MODEL}] {reaction}: {'POSITIVE' if is_pos else 'not positive'}"
        )
    ...
    if events:
        ...
        logger.info(
            f"[LLM-batch: {MODEL}] Positive reactions judged by LLM:\n{details}"
        )
    else:
