...
def get_all_start():
    ...
    if row and row[0]:
        # 日付を00:00に揃えて返す
        return datetime.fromtimestamp(row[0]).replace(hour=0, minute=0, second=0, microsecond=0)
...
def fetch_daily_count(user_id: str, metric: str, start_dt: datetime, end_dt: datetime) -> int:
    """
...
def plot_metric_trends(key: str, label: str, dates: list, data: dict, user_ids: list) -> str:
    """
    折れ線グラフを描画して保存する共通関数
    """
    ...
    for ax, (metric, text) in zip(axes, METRICS):
        ...
        ax.legend(handles, labels, loc='upper center', ncol=len(handles),
                  #bbox_to_anchor=(0.5, 0.98), fontsize='medium')
                  bbox_to_anchor=(0.5, 0.98), fontsize=14)
...
def plot_heatmap_for_period(key: str, label: str, start_dt: datetime, end_dt: datetime, user_ids: list) -> str:
    """
    ...
    for idx, metric in enumerate(heat_metrics):
        ...
        ax_h.set_title(f"{METRICS_DICT[metric]}ヒートマップ (曜日vs時間帯)({label})", fontsize=18)
        ...
        ax_h.set_xticks(np.arange(-.5, 24, 1), minor=True)
        ax_h.set_yticks(np.arange(-.5, 7, 1), minor=True)
        ax_h.grid(which='minor', color='gray', linestyle='-', linewidth=0.5)
        ax_h.grid(False, which='major')
        for (y, x), val in np.ndenumerate(grid):
            ax_h.text(x, y, str(val), ha='center', va='center', fontsize=10, color='black')
...
def main():
    ...
    rows = fetch_user_counts(DB_PATH, yesterday_mid.timestamp(), today_mid.timestamp(), TOP_N)
    ...
    # --- 説明文：DB埋め込み下に表示 ---
    append_paragraph_to_notion(NOTION_PAGE_ID, "以下は、昨日の貢献度ランキングに入ったユーザーの、過去7日間、30日間、全期間の貢献度スコア、スコア構成要素の件数の推移のグラフと、それらのヒートマップです。")
