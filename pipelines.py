...
def process_faq(db):
    ...
    new_count = count_posts_since(
        db, last_ts, channels=[QUESTION_CH], include_replies=True
    )
    THRESHOLD = 10
    if new_count < THRESHOLD:
        logger.info(
            f"[FAQ] Skipping FAQ processing: only {new_count} new posts in QUESTION_CH (< threshold {THRESHOLD})."
        )
        return False
    logger.info(
        f"[FAQ] Starting FAQ processing: {new_count} new posts in QUESTION_CH (>= threshold {THRESHOLD})."
    )
    ...
    for p in posts:
        if p["ts"] >= cutoff_ts:
            filtered.append(p)
    logger.info(
        f"[FAQ] total fetched question posts: {len(posts)}, after window filter: {len(filtered)} (using max_ts={max_ts})"
    )
    if not filtered:
    ...
    logger.info(
        f"[FAQ] Agglomerative clustering produced {len(set(best_labels)) - (1 if -1 in best_labels else 0)} clusters (excluding noise)"
    )
    ...
    for cid in singleton_ids:
        logger.info(
            f"[FAQ] skipping singleton cluster {cid} with only {len(clusters[cid]['post_ids'])} post"
        )
        clusters.pop(cid)
    ...
    for cluster_id, cluster in clusters.items():
        logger.info(
            f"[FAQ] Summarizing cluster {cluster_id} with {len(cluster['texts'])} posts"
        )
        ...
        try:
            ...
            logger.info(
                f"[FAQ] inserted extracted_item id={item_id} for cluster {cluster_id}"
            )
            insert_extracted_item_type(item_id, "faq")
            logger.info(
                f"[FAQ] inserted extracted_item_type for item_id={item_id}, type=faq"
            )
        except Exception as e:
            logger.error(
                f"[FAQ] failed to insert extracted item for cluster {cluster_id}: {e}"
            )
...
def process_trend_topics(db):
    ...
    last_ts = fetch_last_import_ts(db)
    new_count = count_posts_since(
        db, last_ts, channels=TOPIC_CHANNELS, include_replies=False
    )
    THRESHOLD = 10
    if new_count < THRESHOLD:
        logger.info(
            f"[Topics] Skipping trend topics: only {new_count} new posts in topic channels (< threshold {THRESHOLD})."
        )
        return False
    logger.info(
        f"[Topics] Starting trend topics processing: {new_count} new posts in topic channels (>= threshold {THRESHOLD})."
    )
    ...
    valid_clusters = [
        (label, texts) for label, texts in clusters.items() if len(texts) >= 3
    ]
    ...
    cluster_sizes = ", ".join(
        f"{int(label)}:{len(texts)}" for label, texts in valid_clusters
    )
    ...
    for label, cluster_texts in top_clusters:
        ...
        logger.info(
            f"[Topics] extracted topic: {topic_summary}, cluster={label}, size={len(cluster_texts)}"
        )
        try:
            logger.info(
                f"[Topics] inserting trend_topic for cluster={int(label)}, size={len(cluster_texts)}, topic={topic_summary}"
            )
            topic_id = insert_trend_topic(
                db, int(label), topic_summary, len(cluster_texts)
            )
            logger.info(
                f"[Topics] inserted trend_topic id={topic_id}, cluster={label}, size={len(cluster_texts)}, topic={topic_summary}"
            )
        except Exception as e:
            logger.error(
                f"[Topics] failed to insert trend_topic for cluster={label}, size={len(cluster_texts)}, topic={topic_summary}: {e}"
            )
...
def process_info_requests(db):
    ...
    if new_count < THRESHOLD:
        logger.info(
            f"[InfoRequests] Skipping info requests: only {new_count} new posts in topic channels (< threshold {THRESHOLD})."
        )
        return False
    logger.info(
        f"[InfoRequests] Starting info requests processing: {new_count} new posts in topic channels (>= threshold {THRESHOLD})."
    )
    posts = fetch_posts_for_topics(db, TOPIC_CHANNELS, FAQ_WINDOW_DAYS)
    if not posts:
        logger.info("No topic posts found, skipping.")
    ...
    # Filter clusters with at least 3 posts
    valid_clusters = [
        (label, c) for label, c in clusters.items() if len(c["texts"]) >= 3
    ]
    if not valid_clusters:
    ...
    cluster_sizes = ", ".join(
        f"{int(label)}:{len(c['texts'])}" for label, c in valid_clusters
    )
    logger.info(f"[InfoRequests] cluster label:size: {cluster_sizes}")
    for label, cluster in valid_clusters:
        logger.info(
            f"[InfoRequests] summarizing cluster {label} with {len(cluster['texts'])} posts"
        )
        ...
        if not is_request:
            logger.info(
                f"[InfoRequests] skipping cluster {label} as not an information request"
            )
            continue
        ...
        try:
            logger.info(
                f"[InfoRequests] inserting info_request for cluster={label}, size={len(cluster['texts'])}"
            )
            req_id = insert_info_request(db, int(label), payload, len(cluster["texts"]))
            logger.info(
                f"[InfoRequests] inserted info_request id={req_id}, cluster={label}, size={len(cluster['texts'])}"
            )
        except Exception as e:
            logger.error(
                f"[InfoRequests] failed to insert info_request for cluster {label}: {e}"
            )
